...
async function copyListToClipboard(nonFollowersList: readonly Node[]): Promise<void> {
  ...
  alert('Lista copiada al portapeles!');
}
...
function getUnfollowLogForDisplay(
...
function App() {
  ...
  const handleScanFilter = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (state.selectedResults.length > 0) {
      if (!confirm('Cambiar las opciones de filtro borrará los usuarios seleccionados')) {
  ...
  useEffect(() => {
    const onBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
      if (e) {
        e.returnValue = 'Los cambios que realizaste puede que no se guarden';
      }

      // For Safari
      return 'Los cambios que realizaste puede que no se guarden';
    };
    ...
    const scan = async () => {
      ...
      while (hasNext) {
        ...
        if (scrollCycle > 6) {
          ...
          setToast({
            show: true,
            text: 'Pausado 10 segundos para prevenir ser bloqueado temporalmente',
          });
          await sleep(10000);
        }
        setToast({ show: false });
      }
      setToast({ show: true, text: 'Escaneo completado!' });
    };
    scan();
    ...
    const unfollow = async () => {
      ...
      for (const user of state.selectedResults) {
        ...
        if (counter % 5 === 0) {
          setToast({
            show: true,
            text: 'Pausando 5 minutos para prevenir ser bloqueado temporalmente',
          });
          await sleep(300000);
        }
        setToast({ show: false });
      ...
      markup = (
        <section className="flex">
          <aside className="app-sidebar">
            ...
            >
              DEJAR DE SEGUIR ({state.selectedResults.length})
            </button>
          </aside>
        ...
        <section className="flex">
          ...
          <article className="unfollow-log-container">
            {state.unfollowLog.length === state.selectedResults.length && (
              <>
                <hr />
                <div className="fs-large p-medium clr-green">COMPLETADO!</div>
                <hr />
              </>
            )}
              ...
              (entry, index) =>
                entry.unfollowedSuccessfully ? (
                  ...
                  <div className="p-medium clr-red" key={entry.user.id}>
                    Fallo al dejar de seguir a {entry.user.username} [{index + 1}/
        ...
        <header className="app-header">
          ...
          <div className="app-header-content">
            ...
            {state.status === 'scanning' && (
              <input
                title="Seleccionar todo en esta pagina"
                type="checkbox"
...
if (location.hostname !== INSTAGRAM_HOSTNAME) {
  alert('Solo se puede usar en la pagina de Instagram');
} else {
  document.title = 'InstagramNoSeguidos';
